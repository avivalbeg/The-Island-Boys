...
class agent:
    ...
    def __init__(self,grid,location = None,learner = 'Random',viewing = 3,ant_mode = False):
        if location ==None:
        ...
        self.R = 0
        self.grid = grid
        self.knowledge = np.zeros(grid.Map.shape)
        ...
        if learner == 'RandomFinder':
            self.brain = Learner.RandomFinder(self,dynamic = False)
        if learner == 'DynamicRandomFinder':
            self.brain = Learner.RandomFinder(self,dynamic=True)
        if learner == 'DynamicMCTSFinder':
            self.brain = Learner.MCTSFinder(self,dynamic=True)
        if learner == 'MCTSFinder':
            self.brain = Learner.MCTSFinder(self,dynamic=False)
    ...
    def take_step(self,action=None):
        ...
        self.grid.pheremones[self.location[0], self.location[1]] += .02
